...
const AdminMenu: React.FC<AdminMenuProps> = ({ currentAdmin }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Menu.Items className="absolute mt-2 right-0 w-40 px-2 origin-top-right divide-y divide-gray-100 rounded-md bg-secondary shadow-lg ring-1 ring-black/5 focus:outline-none">
          <div className="p-1">
            {currentAdmin ? (
              <Menu.Item>
                {({ active }) => (
